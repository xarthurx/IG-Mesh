...
namespace igmGH
{
    public class IGM_isoline_scalar : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetDataList(1, mesh_scalar)) { return; }
            if (!DA.GetDataList(2, iso_t)) { return; }

            foreach (var t in iso_t)
            {
                if (t < 0 || t > 1)
                    AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "parameter t should be in the range [0, 1].");
            }
