...
template <typename T>
void GeoLib::computeIsoPts(const Matrix<T, -1, -1>& V,
                                    const MatrixXi& F,
                                    const Vector<T, -1>& meshScalar,
                                    const Vector<T, -1>& isoValue,
                                    map<T, Matrix<T, -1, -1>>& isoLinePts,
                                    bool sorted) {
  isoLinePts.clear();
...
// template helper to put definition in the cpp file
template void GeoLib::solveScalarField(const Matrix<double, -1, -1>& V,
                                       const MatrixXi& F,
                                       const VectorXi& con_idx,
                                       const Vector<double, -1>& con_val,
                                       Vector<double, -1>& meshScalar);

template void GeoLib::solveScalarField(const Matrix<float, -1, -1>& V,
                                       const MatrixXi& F,
                                       const VectorXi& con_idx,
                                       const Vector<float, -1>& con_val,
                                       Vector<float, -1>& meshScalar);

template void GeoLib::computeIsoPts(
    const Matrix<double, -1, -1>& V, const MatrixXi& F,
    const Vector<double, -1>& meshScalar, const Vector<double, -1>& isoValue,
    map<double, Matrix<double, -1, -1>>& isoLinePts, bool sorted);

template void GeoLib::computeIsoPts(
    const Matrix<float, -1, -1>& V, const MatrixXi& F,
    const Vector<float, -1>& meshScalar, const Vector<float, -1>& isoValue,
    map<float, Matrix<float, -1, -1>>& isoLinePts, bool sorted);
